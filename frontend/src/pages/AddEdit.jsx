...
export function AddEdit() {
  ...
  const addBlog = async () => {
    if (blog.title && blog.description && blog.category) {
      ...
      try {
        ...
        const res = await axios.post("http://localhost:3001/blogs", edBlog, {
  ...
  return (
    ...
    >
      ...
      <Button onClick={addBlog} variant="contained">
        add blog
      </Button>
      {err ? (
        <div style={{ color: "red" }}>please fill required fields</div>
      ) : (
        ""
      )}
    </div>
